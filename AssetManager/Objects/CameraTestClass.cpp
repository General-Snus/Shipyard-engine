...
void MainCamera::Update(float aTimeDelta)
{
	...
	{
		myTransform = myTransform.CreateRotationAroundY(2.f * aTimeDelta) * myTransform;
	}
	...
	if (GetAsyncKeyState(VK_SPACE))
	{
		for (int i = 1; i <= 3; i++)
		{
			myTransform(4, i) += mdf * aTimeDelta * myDirections.y[i - 1];
		}
	}
...
Vector3f MainCamera::GetPosition() const
{
