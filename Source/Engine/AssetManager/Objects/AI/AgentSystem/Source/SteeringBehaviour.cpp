...
{
	OPTICK_EVENT();
	const Vector3f acceleration = (targetVelocity - kinematic->ph_velocity) / timeToMatch;
                                   ...
                                   const Vector3f& position, SeparationSettings                settings)
{
	...
	if (distance < settings.threshold)
	{
		const float strength = std::min(settings.decayCoefficient / (distance * distance), settings.maxAcceleration);
...
{
	OPTICK_EVENT();
	const Vector3f AntiNormal = -wallNormal;
