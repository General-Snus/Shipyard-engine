...
REFL_AUTO(type(SpotLight), field(Power), field(Range), field(InnerConeAngle), field(OuterConeAngle))
...
struct aligned_int // I love 16 byte alignment
{
	...
	void operator=(aligned_int val)
	{
		value = val.value;
	}

	aligned_int(std::initializer_list<int> v) : value(static_cast<int>(*v.begin()))
	{
	}
