...
public:
	...
	template <class T> void SetUpdatePriority(const UpdatePriority aPriority);
	void Update();
...
#pragma region GOM
template<typename T>
inline auto ReflectedComponentName = refl::reflect<T>().name.str();
...
template <class T> void GameObjectManager::SetUpdatePriority(const UpdatePriority aPriority)
{
	OPTICK_EVENT();
	if (!myComponentManagers.contains(ReflectedComponentName<T>))
	{
