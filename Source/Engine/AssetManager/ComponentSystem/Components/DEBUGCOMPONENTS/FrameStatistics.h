...
#define SKIPRATE 1
	...
	float m_committedMemory		[MAX_DATA_COUNT];     // Bytes of memory currently committed/in-flight
	float m_totalMemory			[MAX_DATA_COUNT];         // Total bytes of memory used by the allocators
	float m_totalPages			[MAX_DATA_COUNT];          // Total page count
	float m_peakCommitedMemory	[MAX_DATA_COUNT];  // Peak commited memory value since last reset
