...
template<class T>
bool AssetManager::ForceLoadAsset(const std::filesystem::path& identifierPath, const bool useExact, std::shared_ptr<T>& outAsset)
{
	...
	if (useExact)
	{
		Identifier = std::filesystem::relative(identifierPath, AssetPath); //Might invoke unexpected behaviour but stops you from duplication
