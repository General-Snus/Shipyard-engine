...
PostProcessPixelOutput main(SS_VStoPS input)
{
    ...
    const float intensity = g_GraphicsSettings.GSB_AO_intensity;
    const float scale = g_GraphicsSettings.GSB_AO_scale;
    const float bias = g_GraphicsSettings.GSB_AO_bias;
    const float radius = g_GraphicsSettings.GSB_AO_radius;
    const float offset = g_GraphicsSettings.GSB_AO_offset;
