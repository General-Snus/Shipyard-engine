...
PostProcessPixelOutput main(BRDF_VS_to_PS input)
{
    ...
    const float4 centerNormalDepth = float4(textureHeap[g_defaultMaterial.normalTexture].Sample(defaultSampler, input.UV).rgb, 0);
    ...
    { 
        ...
        { 
            ...
            float4 neighborNormalDepth = float4(textureHeap[g_defaultMaterial.normalTexture].Sample(defaultSampler, tex).rgb, 0);
