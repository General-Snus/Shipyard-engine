...
)
{
	...
	for	(int i = 0; i < 4; ++i)
    {
        ...
        outputData.Position = mul(g_FrameBuffer.view, particle.Position); // from world to view space
        outputData.Position.xy += offsets[i] * particle.Scale.xy; // Scale in space that is looking at camera, aka billboard
		outputData.Position = mul(g_FrameBuffer.projection, outputData.Position);  // apply projection
