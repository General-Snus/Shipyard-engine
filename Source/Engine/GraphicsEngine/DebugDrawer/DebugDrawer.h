...
struct DebugVertex
{
	DebugVertex(Vector3f aPosition,Vector3f aColor) : Position(aPosition.x,aPosition.y,aPosition.z,1),Color(aColor.x,aColor.y,aColor.z,1) {};
...
public:
	...
	PrimitiveHandle AddDebugLine(const Vector3f& aStart,const Vector3f& aFinish,const Vector3f& aColor);
	PrimitiveHandle AddDebugGizmo(const Vector3f& aCenter,const float aLength);
	...
	PrimitiveHandle AddDebugBox(const Vector3f& aMin,const Vector3f& aMax/*, Rotator*/,const Vector3f& aColor);
	...
	PrimitiveHandle AddDebugGrid(const Vector3f& aCenter,const float anExtent,const unsigned int someNumCells/*, Rotator*/,const  Vector3f& aColor);
...
private:
	PrimitiveHandle CreatePrimitiveHandle(const Primitive& aPrimitive);
