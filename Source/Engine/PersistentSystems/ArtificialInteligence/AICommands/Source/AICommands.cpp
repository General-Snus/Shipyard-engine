...
namespace GeneralizedAICommands
{
	...
	bool IsTargetInRange(GameObject input)
	{
		...
		const GameObject obj = AIPollingManagerInstance.GetStation<MultipleTargets_PollingStation>("Targets")->
GetClosestTarget(
		...
		if (distance < myStats.myVisionRange)
		{
			...
			if (Raycast(myTransform.GetPosition(), direction, hit, input,
			            static_cast<Layer>(static_cast<int>(Layer::Entities) | static_cast<int>(Layer::Default))))
			{
				if (hit.objectHit.GetID() == obj.GetID())
				{
					color = {0, 1, 0};
					GraphicsEngineInstance.debugDrawer.AddDebugLine(myTransform.GetPosition(), closestTarget, color,
					                                                0.01f);
					return true;
				}
			}
			GraphicsEngineInstance.debugDrawer.AddDebugLine(myTransform.GetPosition(), closestTarget, color, 0.01f);
		}
		return false;
	}
