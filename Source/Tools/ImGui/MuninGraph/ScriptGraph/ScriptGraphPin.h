...
class ScriptGraphPin : public NodeGraphPin<ScriptGraphNode, ScriptGraphSchema>
{
    ...
    ScriptGraphPin(const std::shared_ptr<ScriptGraphNode> &anOwner, const std::string &aLabel, PinDirection aDirection,
                   PinIcon anIcon, ScriptGraphPinType aType)
  ...
  protected:
     ...
     */
    template <typename DataType> void InitDataBlock()
    {
  ...
  public:
    ...
    static ScriptGraphPin CreateDataPin(const std::shared_ptr<ScriptGraphNode> &aOwner, const std::string &aLabel,
                                        ScriptGraphPinType aType, PinIcon anIcon, PinDirection aDirection,
                                        bool hideLabelOnNode = false)
    {
    ...
    static ScriptGraphPin CreatePin(const std::shared_ptr<ScriptGraphNode> &aOwner, const std::string &aLabel,
                                    ScriptGraphPinType aType, PinIcon anIcon, PinDirection aDirection,
                                    bool hideLabelOnNode = false)
    {
        ScriptGraphPin Pin(aOwner, aLabel, aDirection, anIcon, aType);
    ...
    template <typename DataType> bool GetData(DataType &outData) const
    {
#if _DEBUG
        const std::type_index &RequestedType = typeid(DataType);
        assert(RequestedType == myData.TypeData->GetType() &&
               "Cannot GetData of another type than the one that is stored!");
#endif
    ...
    template <typename DataType> void SetData(DataType data)
    {
#if _DEBUG
        const std::type_index &RequestedType = typeid(DataType);
        assert(RequestedType == myData.TypeData->GetType() &&
               "Cannot SetData of another type than the one that is stored!");
    ...
    FORCEINLINE ScriptGraphPinType GetType() const
    {
        return myType;
    }
    FORCEINLINE bool IsLabelVisible() const
    {
        return myLabelVisible;
    }
    virtual FORCEINLINE std::type_index GetDataType() const
    {
        if (myData.TypeData)
            return myData.TypeData->GetType();
