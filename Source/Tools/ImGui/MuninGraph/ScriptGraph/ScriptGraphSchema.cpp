...
std::shared_ptr<ScriptGraphNode> ScriptGraphSchema::CreateNode(const RegisteredNodeClass &aClass)
{
    ...
    if (RegisterNode(newNode))
    {
        if (newNode->GetNodeType() == ScriptGraphNodeType::Event)
        {
            ...
            const std::shared_ptr<ScriptGraphEventNode> &newEventNode =
                std::static_pointer_cast<ScriptGraphEventNode>(newNode);
            ...
            if (entryHandle.empty())
            {
                throw std::runtime_error("Node Entry Handle is empty! Please override the appropriate function in the "
                                         "node or give it an appropriate name!");
            }
...
bool ScriptGraphSchema::CanCreateEdge(size_t aSourcePinId, size_t aTargetPinId, std::string &outMessage) const
{
    ...
    if (sourcePin.GetPinType() != ScriptGraphPinType::Exec && targetPin.GetPinType() != ScriptGraphPinType::Exec)
    {
        ...
        {
            outMessage = std::format("{} is not compatible with {}.", sourcePin.GetPinDataType()->GetFriendlyName(),
                                     targetPin.GetPinDataType()->GetFriendlyName());
            return false;
        }
    }

    if ((sourcePin.GetPinType() == ScriptGraphPinType::Variable ||
         sourcePin.GetPinType() == ScriptGraphPinType::Data) &&
...
size_t ScriptGraphSchema::CreateDynamicExecPin(ScriptGraphNode *aNode, std::string_view aLabel, PinDirection aDirection)
{
    ...
    const size_t aPinUID =
        aNode->AddPin(ScriptGraphPin::CreateDynamicPin(aNode, aLabel, ScriptGraphPinType::Exec, aDirection));
...
{
    const size_t aPinUID =
        aNode->AddPin(ScriptGraphPin::CreateDynamicDataPin(aNode, aLabel, ScriptGraphPinType::Data, aDirection, aType));
    GetMutableGraph()->myPins.emplace(aPinUID, &aNode->GetMutablePin(aPinUID));
    return aPinUID;
}
...
void ScriptGraphSchema::CheckCyclicLink(const ScriptGraphNode *aNode, const ScriptGraphNode *aBaseNode,
...
void ScriptGraphSchema::CreateEdgeInternal(ScriptGraphPin &aSourcePin, ScriptGraphPin &aTargetPin) const
{
    ...
    graph->myEdges.insert(
        {newEdgeUID, {{newEdgeUID, aSourcePin.GetUID(), aTargetPin.GetUID()}, aSourcePin.GetPinType()}});
