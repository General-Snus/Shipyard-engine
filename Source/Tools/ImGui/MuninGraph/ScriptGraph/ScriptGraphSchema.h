...
class ScriptGraphSchema
    : public NodeGraphSchema<ScriptGraphSchema, ScriptGraph, ScriptGraphNode, ScriptGraphEdge, ScriptGraphPin>
{
  public:
    ScriptGraphSchema(ScriptGraph *aGraph) : NodeGraphSchema(aGraph)
    {
    }
    ...
    template <typename T> std::shared_ptr<ScriptGraphNode> AddNode()
    {
    ...
    size_t CreateDynamicDataPin(size_t aNodeId, std::string_view aLabel, PinDirection aDirection,
                                std::type_index aType);
    size_t CreateDynamicDataPin(ScriptGraphNode *aNode, std::string_view aLabel, PinDirection aDirection,
                                std::type_index aType);

    template <typename T> void CreateDynamicDataPin(size_t aNodeId, std::string_view aLabel, PinDirection aDirection)
    {
    ...
    bool AddVariable(std::string_view aVariableName, const TypedDataContainer &aDefaultValue,
                     int aFlags = ScriptGraphVariableFlag_None);
    ...
    FORCEINLINE const std::unordered_map<std::string, size_t, string_hash, std::equal_to<>> &GetEntryPoints() const
    {
        return GetGraph()->myEntryPoints;
    }
    FORCEINLINE const std::unordered_map<std::string, ScriptGraphVariable, string_hash, std::equal_to<>> &GetVariables()
        const
    {
        return GetGraph()->myVariables;
    }
