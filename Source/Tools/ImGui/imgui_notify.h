...
#define NOTIFY_USE_DISMISS_BUTTON                                                                                      \
	true					  // If true, a dismiss button will be rendered in the top right corner of the toast
...
#define NOTIFY_RENDER_OUTSIDE_MAIN_WINDOW                                                                              \
	true // If true, the notifications will be rendered in the corner of the monitor, otherwise in the corner of the
		 // main window
...
class ImGuiToast
{
  private:
	...
	std::function<void()> onButtonPress =
		nullptr; // A lambda variable, which will be executed when button in notification is pressed
  ...
  public:
	 ...
	 */
	inline void setOnButtonPress(const std::function<void()> &buttonPress)
	{
		this->onButtonPress = buttonPress;
	}
  ...
  public:
	...
	inline float getFadePercent()
	{
		...
		{
			return (1.f - (((float)elapsed - (float)NOTIFY_FADE_IN_OUT_TIME - (float)this->dismissTime) /
  ...
  public:
	 ...
	 * @param dismissTime The time in milliseconds after which the toast should be dismissed. Default is
	...
	/**
	 * @brief Constructor for creating a new ImGuiToast object with a specified type, dismiss time, title format,
	 * content format and a button.
	 *
	 * @param type The type of the toast message.
	 * @param dismissTime The time in milliseconds before the toast message is dismissed.
 ...
 */
inline int Notify(const ImGuiToast &toast)
{
	notifications.push_back(toast);
	return static_cast<int>(notifications.size()-1);
}
...
inline void RenderNotifications()
{
	...
	for (int i = 0; i < notifications.size(); ++i)
	{
		...
		SetNextWindowPos(ImVec2(monitor.WorkPos.x + monitor.WorkSize.x - NOTIFY_PADDING_X,
								monitor.WorkPos.y + monitor.WorkSize.y - NOTIFY_PADDING_Y - height),
						 ImGuiCond_Always, ImVec2(1.0f, 1.0f));
#else
		// Set notification window position to bottom right corner of the main window, considering the main window size
		// and location in relation to the display
		ImVec2 mainWindowPos = GetMainViewport()->Pos;
		SetNextWindowPos(ImVec2(mainWindowPos.x + mainWindowSize.x - NOTIFY_PADDING_X,
