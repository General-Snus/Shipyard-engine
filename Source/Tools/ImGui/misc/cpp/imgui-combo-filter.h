...
namespace ImGui
{
	...
	namespace Internal
	{
		...
		bool ComboAutoSelectEX(const char* combo_label, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiComboFlags flags)
		{
			...
			bool Override_buffer_changed = false;
			...
			if (combo_data->CurrentSelection != selected_item)
			{ 
				combo_data->SetNewValue(item_getter(items, selected_item), selected_item);
				Override_buffer_changed = true;
			}
			...
			const bool buffer_changed = Override_buffer_changed | InputTextEx("##inputText", NULL, combo_data->InputText, ComboData::StringCapacity, ImVec2(0, 0), ImGuiInputTextFlags_AutoSelectAll, NULL, NULL);
			PopStyleColor(2);
			PopStyleVar(1);
			PopItemWidth();
...
template<typename T>
inline int autoselect_search(const ImGui::ComboAutoSelectSearchCallbackData<T>& cbd)
{
...
inline int autoselect_search_vector(const ImGui::ComboAutoSelectSearchCallbackData<const std::vector<std::string>&>& cbd)
{
	if (cbd.SearchString[0] == '\0')
		return -1;
...
template<typename T>
inline void filter_search(const ImGui::ComboFilterSearchCallbackData<T>& cbd)
{
