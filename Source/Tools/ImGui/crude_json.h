...
#undef CRUDE_MAX2
    ...
    class replacementAlignedStorage
    {
      private:
        alignas(max_align) std::byte t_buff[max_size];
    };

    using storage_t = replacementAlignedStorage;
...
template <> inline boolean &value::get<boolean>()
{
    CRUDE_ASSERT(m_Type == type_t::boolean);
    return *boolean_ptr(m_Storage);
}
...
template <> inline const object *value::get_ptr<object>() const
{
...
template <> inline const array *value::get_ptr<array>() const
{
...
template <> inline const string *value::get_ptr<string>() const
{
...
template <> inline const boolean *value::get_ptr<boolean>() const
{
...
template <> inline const number *value::get_ptr<number>() const
{
...
template <> inline object *value::get_ptr<object>()
{
    if (m_Type == type_t::object)
        return object_ptr(m_Storage);
    else
...
template <> inline array *value::get_ptr<array>()
{
    if (m_Type == type_t::array)
        return array_ptr(m_Storage);
    else
...
template <> inline string *value::get_ptr<string>()
{
    if (m_Type == type_t::string)
        return string_ptr(m_Storage);
    else
...
template <> inline boolean *value::get_ptr<boolean>()
{
    if (m_Type == type_t::boolean)
        return boolean_ptr(m_Storage);
    else
...
template <> inline number *value::get_ptr<number>()
{
    if (m_Type == type_t::number)
        return number_ptr(m_Storage);
    else
