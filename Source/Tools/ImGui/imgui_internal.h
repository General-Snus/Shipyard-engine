...
typedef int ImGuiDataAuthority; // -> enum ImGuiDataAuthority_      // Enum: for storing the source authority (dock node
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char> InitialTextA; // value to revert to when pressing Escape = backup of end-user buffer at the time of
    ...
    bool ReloadUserBuf; // force a reload of user buf so it may be modified externally. may be automatic in future
                        // version.
...
struct ImGuiNavItemData
{
    ...
    ImGuiSelectionUserData SelectionUserData; // I+Mov    // Best candidate SetNextItemSelectionUserData() value. Valid
                                              // if (InFlags & ImGuiItemFlags_HasSelectionUserData)
...
{
    // [Internal]
    ImGuiDockNodeFlags_DockSpace = 1 << 10, // Saved // A dockspace is a node that occupy space within an existing user
                                            // window. Otherwise the node is floating and create its own window.
    ImGuiDockNodeFlags_CentralNode = 1 << 11, // Saved // The central node has 2 main properties: stay visible when
    ...
    ImGuiDockNodeFlags_HiddenTabBar =
        1 << 13, // Saved // Tab bar is hidden, with a triangle in the corner to show it again (NB: actual tab-bar
    ...
    ImGuiDockNodeFlags_DockedWindowsInFocusRoute =
        1 << 18, //       // Any docked window will be automatically be focus-route chained
    ...
    // Those are not exposed in public because the desirable sharing/inheriting/copy-flag-on-split behaviors are quite
    // difficult to design and understand. The two public flags
    // ImGuiDockNodeFlags_NoDockingOverCentralNode/ImGuiDockNodeFlags_NoDockingSplit don't have those issues.
    ...
    ImGuiDockNodeFlags_NoDockingOverEmpty =
        1
        << 22, //       // Disable this node from being docked over an empty node (e.g. DockSpace with no other windows)
...
struct IMGUI_API ImGuiDockNode
{
    ...
    bool HasCloseButton : 1; // Provide space for a close button (if any of the docked window has one). Note that button
                             // may be hidden on window without one.
...
// We don't store style.Alpha: dock_node->LastBgColor embeds it and otherwise it would only affect the docking tab,
// which intuitively I would say we don't want to.
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    float LastAlpha;
    bool
        LastFocusedHadNavWindow; // Instead of maintaining a LastFocusedWindow (which may harder to correctly maintain),
                                 // we merely store weither NavWindow != NULL last time the viewport was focused.
...
struct ImGuiContext
{
    ...
    ImGuiWindow *MovingWindow; // Track the window we clicked on (in order to preserve focus). The actual window that is
                               // moved is generally MovingWindow->RootWindowDockTree.
    ...
    int ViewportFocusedStampCount;       // Every time the front-most window changes, we stamp its viewport with an
    ...
    bool NavJustMovedToHasSelectionData; // Copy of move result's InFlags & ImGuiItemFlags_HasSelectionUserData). Maybe
                                         // we should just store ImGuiNavItemData.
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_Unsorted =
        1
        << 22, // [Docking] Trailing tabs with the _Unsorted flag will be sorted based on the DockOrder of their Window.
};
...
IMGUI_API void RenderNavHighlight(const ImRect &bb, ImGuiID id,
                                  ImGuiNavHighlightFlags flags = ImGuiNavHighlightFlags_None); // Navigation highlight
...
// inline bool   IsKeyPressedMap(ImGuiKey key, bool repeat = true)                   { IM_ASSERT(IsNamedKey(key));
// return IsKeyPressed(key, repeat); } // Removed in 1.87: Mapping from named key is always identity!
