...
template <class T> inline Vector4<T> MinVector(const Vector4<T> &a, const Vector4<T> &b)
{
	return Vector4<T>((a < b.x) ? a.x : b.x, (a < b.y) ? a.y : b.y, (a < b.z) ? a.z : b.z, (a < b.w) ? a.w : b.w);
}

template <class T> inline Vector4<T> MaxVector(const Vector4<T> &a, const Vector4<T> &b)
{
	return Vector4<T>((a > b.x) ? a.x : b.x, (a > b.y) ? a.y : b.y, (a > b.z) ? a.z : b.z, (a > b.w) ? a.w : b.w);
}
...
template <class T> inline Vector4<T> &Vector4<T>::operator=(const T &aTypeT)
{
	x, y, z, w = aTypeT;
	return *this;
}

template <class T> inline Vector4<T> Vector4<T>::operator-() const
{
...
template <class T> inline Vector4<T>::Vector4(const Vector3<T> &aVector, const T &aW)
{
	x = aVector.x;
	y = aVector.y;
	z = aVector.z;
...
template <class T>
inline Vector4<T>::Vector4(const VectorX<T, 4> &aVector) : x(aVector[0]), y(aVector[1]), z(aVector[2]), w(aVector[3])
{
}

template <class T> template <class U> inline Vector4<T>::operator U() const
{
	return {x, y, z, w};
}

template <class T> inline T &Vector4<T>::operator[](int value)
{
...
template <class T> inline const T &Vector4<T>::operator[](int value) const
{
	assert(value >= 0 && value <= 4);
	switch (value)
	{
...
template <> struct std::hash<Vector4f>
{
