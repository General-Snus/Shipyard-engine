...
Quaternion<T> Quaternion<T>::LookAt(const Vector3<T>& aDirection, const Vector3f& aUp)
{
	...
	const Vector3f right = up.Cross(forward).GetNormalized();
	up = forward.Cross(right);

	return Quaternion(Matrix3x3<T>(right, up, forward));

	////from world forward to Object forward
	//const Quaternion f2d = RotationFromTo(Math::GlobalFwd,forward);
	//
	////what direction is the new object up?
	//const Vector3f objectUp = f2d * Math::GlobalUp;
