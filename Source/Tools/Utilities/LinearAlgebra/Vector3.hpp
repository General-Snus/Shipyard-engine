...
template <class T> inline constexpr Vector3<T> &Vector3<T>::operator=(const T &aTypeT)
{
	x, y, z = aTypeT;
	return *this;
}
...
template <class T> inline constexpr Vector3<T> Vector3<T>::operator-() const
{
	return Vector3<T>(-x, -y, -z);
}

template <class T> inline constexpr T Vector3<T>::LengthSqr() const
{
	return static_cast<T>(x * x + y * y + z * z);
}

template <class T> inline constexpr T Vector3<T>::Length() const
{
	return static_cast<T>(std::sqrt(LengthSqr()));
}

template <class T> inline constexpr Vector3<T> Vector3<T>::GetNormalized() const
{
...
template <class T> inline constexpr void Vector3<T>::Normalize()
{
	const T len = 1 / Length();
...
template <class T> inline constexpr T Vector3<T>::Dot(const Vector3<T> &aVector) const
{
	return x * aVector.x + y * aVector.y + z * aVector.z;
}

template <class T> inline constexpr Vector3<T> Vector3<T>::Cross(const Vector3<T> &aVector) const
{
	return Vector3<T>(y * aVector.z - aVector.y * z, z * aVector.x - aVector.z * x, x * aVector.y - aVector.x * y);
}

template <class T> constexpr Vector3<float> Vector3<T>::right()
{
	return Vector3<float>(1, 0, 0);
}

template <class T> constexpr Vector3<float> Vector3<T>::up()
{
	return Vector3<float>(0, 1, 0);
}

template <class T> constexpr Vector3<float> Vector3<T>::forward()
{
	return Vector3<float>(0, 0, 1);
}

template <class T> inline constexpr Vector3<T>::Vector3()
{
	x = 0;
	y = 0;
	z = 0;
...
template <class T> inline constexpr Vector3<T>::Vector3(const T &aX, const T &aY, const T &aZ)
{
	x = aX;
	y = aY;
	z = aZ;
...
template <class T> inline constexpr Vector3<T>::Vector3(const T &aValue)
{
	x = aValue;
	y = aValue;
	z = aValue;
...
template <class T>
inline Vector3<T>::Vector3(const VectorX<T, 3> &aVector) : x(aVector[0]), y(aVector[1]), z(aVector[2])
{
}

template <class T> template <class U> inline constexpr Vector3<T>::operator U() const
{
	return {x, y, z};
}

template <class T> inline constexpr T &Vector3<T>::operator[](int value)
{
...
template <class T> std::ostream &operator<<(std::ostream &anOstream, const Vector3<T> &aVector)
{
	anOstream << "{x: " << aVector.x << ", y: " << aVector.y << ", z: " << aVector.z << "}";
	return anOstream;
}

template <class T> inline constexpr const T &Vector3<T>::operator[](int value) const
{
	assert(value >= 0 && value <= 3);
	switch (value)
	{
