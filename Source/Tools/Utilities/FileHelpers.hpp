...
inline bool IsPathInsideDirectory(const std::filesystem::path& path, const std::filesystem::path& directoryPath)
{
	...
	auto absPath = canonical(path);
	auto absDirectoryPath = canonical(directoryPath);

	// Use std::mismatch to compare the paths
	auto [pathIt, dirPathIt] = std::mismatch(absPath.begin(), absPath.end(), absDirectoryPath.begin(),
	                                         absDirectoryPath.end());
