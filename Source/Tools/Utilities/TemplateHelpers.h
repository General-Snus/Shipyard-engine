...
template <typename T, class Deleter = std::default_delete<T>> inline constexpr const bool is_smart_pointer = false;
...
template <typename T> inline constexpr const bool is_smart_pointer<std::unique_ptr<T>> = true;
template <typename T> inline constexpr const bool is_smart_pointer<const std::unique_ptr<T>> = true;
template <typename T> inline constexpr const bool is_smart_pointer<std::shared_ptr<T>> = true;
template <typename T> inline constexpr const bool is_smart_pointer<const std::shared_ptr<T>> = true;
...
template <NotSmartPointerType T> constexpr T unwrapPointer(const T &not_smart_pointer)
{
    return not_smart_pointer;
}
template <NotSmartPointerType T> constexpr T &unwrapPointer(T &&not_smart_pointer)
{
    return std::forward<T &>(not_smart_pointer);
}
template <NotSmartPointerType T> constexpr T &unwrapPointerCreateIfAbsent(T &&not_smart_pointer)
{
    return std::forward<T &>(not_smart_pointer);
}
template <SmartPointerType T> constexpr typename T::element_type &unwrapPointer(T &smart_pointer)
{
    return std::forward<typename T::element_type &>(*smart_pointer.get());
}
template <SmartPointerType T> constexpr typename T::element_type &unwrapPointer(const T &smart_pointer)
{
    return std::forward<typename T::element_type &>(*smart_pointer.get());
}
...
template <typename T> inline constexpr const bool is_array = false;
template <typename T, std::size_t N> inline constexpr const bool is_array<std::array<T, N>> = true;
template <typename T, std::size_t N> inline constexpr const bool is_array<const std::array<T, N>> = true;
...
template <typename... Trest> struct unique_types;
...
template <typename T, typename... Ts> constexpr bool Contains = (std::is_same<T, Ts>{} || ...);
template <typename Subset, typename Set> constexpr bool IsSubsetOf = false;
