...
#define ThreadPoolInstance ServiceLocator::Instance().GetService<ThreadPool>()
class ThreadPool : public Singleton
{
  ...
  public:
    ...
    template <typename F, typename... Args> auto SubmitWork(F &&f, Args &&...args) -> std::future<decltype(f(args...))>
    {
        std::packaged_task<decltype(f(args...))()> task(std::forward<F>(f), std::forward<Args>(args)...);

        auto future = task.get_future();
        auto wrap = [task = std::move(task)]() mutable { task(); };
    ...
    class ThreadWorker
    {
      public:
        ...
        void operator()() const
        {
            ...
            {
                pool->activeThreads--;
                pool->conditional_var.wait(lock, [this] { return pool->ShouldClose || !pool->workerQueue.empty(); });
                pool->activeThreads++;
