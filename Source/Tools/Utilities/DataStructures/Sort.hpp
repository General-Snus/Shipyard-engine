...
void QuickSort(std::vector<T>& aVector, int low, int high)
{
	...
	{
		int pivot = internal_partition<T>(aVector, low, high);

		QuickSort<T>(aVector, low, pivot); //Could be done sending iterators instead
		QuickSort<T>(aVector, pivot + 1, high);
	}
}
...
{
	const int pivot = internal_partition<T>(aVector, 0, static_cast<int>(aVector.size()) - 1);

	QuickSort<T>(aVector, 0, pivot);
	// Start a new sort on the left partition, will run once in this overloaded function
	QuickSort<T>(aVector, pivot + 1, static_cast<int>(aVector.size()) - 1);
	// Start a new sort on the right partition, will run once in this overloaded function
}
