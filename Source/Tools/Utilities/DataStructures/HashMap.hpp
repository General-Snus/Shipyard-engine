...
template <class Key, class Value>
HashMap<Key, Value>::HashMap() : HashMap(500)
{
}
...
bool HashMap<Key, Value>::Insert(const Key& aKey, const Value& aValue)
{
	...
	while (myEntries[index]->state & InUse && !IsEqual(myEntries[index]->key, aKey))
	{
	...
	if (myEntries[index]->state != InUse)
	{
		lockedElements++;
	}
...
{
	return MurmurHash2A(aKey.c_str(), static_cast<int>(aKey.size()), 3984759834);
}
