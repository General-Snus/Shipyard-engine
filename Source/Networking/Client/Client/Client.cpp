...
{
    unsigned long ul = 1;
    ioctlsocket(mySocket, FIONBIO, (unsigned long *)&ul);
    ...
    //     Editor::Get().myChatMessages.emplace_back(message->ReadMessage());
    // }
    // break;
    ...
    //     //         material->Init(mat);
    //     //         AssetManager::Get().AddAsset(material);
...
bool Client::SendOK()
{
    std::string username = "User" + std::to_string(RandomEngine::RandomInRange(0, 999));
