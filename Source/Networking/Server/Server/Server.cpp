...
void Server::Update()
{
    ...
    while (true)
    {
        ...
        case eNetMessageType::ChatMessage: {
            ...
            std::string userMessage = "User: " + std::to_string(recieveID) + " With name: " + myUsersnames[recieveID] +
            ...
            for (const auto &[id, name] : myUsersnames)
            {
                ...
                if (sendto(mySocket, outbound, MAX_NETMESSAGE_SIZE, 0, reinterpret_cast<sockaddr *>(&myUsersIPs[id]),
                           length) == SOCKET_ERROR)
                {
        ...
        case eNetMessageType::Quit: {
            ...
            std::string userMessage =
                "User: " + std::to_string(recieveID) + " With name: " + myUsersnames[recieveID] + " Left the server";
            std::cout << userMessage << std::endl;
