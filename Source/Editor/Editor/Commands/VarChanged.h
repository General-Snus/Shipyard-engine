...
{
public:
	PointerVarChanged(var* value, const var oldVal, const var newValue,
...
inline bool PointerVarChanged< var>::Merge(std::shared_ptr<BaseCommand>& ptr)
{
...
template <typename var>
inline PointerVarChanged< var>::PointerVarChanged(var* value, const var oldVal,
...
template <typename var>
inline std::string PointerVarChanged<var>::GetDescription() const
{
	if constexpr (refl::trait::is_reflectable_v<var> || refl::trait::is_container_v<var> || refl::runtime::detail::is_ostream_printable_v<char, var>)
	{
	...
	{
		const std::string outMessage =
			std::format("{}:{} changed.", refl::reflect<var>().name.str(), m_VariableName);
		return outMessage;
	}
}
#pragma endregion
