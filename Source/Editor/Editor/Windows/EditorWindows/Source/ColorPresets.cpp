...
void ColorPresets::RenderImGUi()
{
	...
	ImGui::Begin("ColorPresets", &m_KeepWindow);
	{
		...
		for (auto& [Name, Value] : ColorManager::m_NamedColor)
		{
			...
			ImGui::NextColumn();
			ImGui::ColorEdit4(("##" + Name).c_str(), &Value, ImGuiColorEditFlags_NoInputs); 
			ImGui::NextColumn();
		...
		else
		{
			...
			if (ImGui::ComboAutoSelect("Blend value 1", selected_item1, keys, item_getter1, autoselect_search_vector, ImGuiComboFlags_HeightSmall))
			{

			}

			static int selected_item2 = -1;
			if (ImGui::ComboAutoSelect("Blend value 2", selected_item2, keys, item_getter1, autoselect_search_vector, ImGuiComboFlags_HeightSmall))
			{
