...
ContentDirectory::ContentDirectory() :
	...
	EditorWindow("Directory", ImGuiWindowFlags_MenuBar)
{
}

void ContentDirectory::Initialize()
{
		...
		if (ImGui::Selectable("Open"))
		{
			if (auto asset = GetEngineResources().TryLoadAsset(path))
			{
				auto newWindow = std::make_shared<CustomFuncWindow>(
					...
					0,
					std::bind(&AssetBase::InspectorView, asset)
				);
	...
	if (!keyTerm.empty())
	{
		for (const auto& it : std::filesystem::recursive_directory_iterator(GetEngineResources().Directory()))
		{
			...
			is_directory(it.path())
				? m_CurrentDirectoryPaths.emplace_back(fileName, relativePath, true)
				: m_CurrentDirectoryPaths.emplace_back(fileName, relativePath, false);
		}
	}
	else
	{
