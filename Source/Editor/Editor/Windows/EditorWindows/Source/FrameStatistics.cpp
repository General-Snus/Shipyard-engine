...
void FrameStatistics::RenderImGUi()
{
    ...
    else
    {
        ...
        m_peakCommitedMemory[myDataIndex] = (float)mem.peakCommitedMemory;
        m_peakTotalMemory[myDataIndex] = (float)mem.peakTotalMemory;
...
double App_CPU_Usage()
{
    ...
    percent = ((double)sys.QuadPart - (double)lastSysCPU.QuadPart) + (user.QuadPart - lastUserCPU.QuadPart);
