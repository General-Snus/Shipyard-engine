...
ImageViewer::ImageViewer(const char* name, std::shared_ptr<Texture> aTexture) :
	texture(aTexture), EditorWindow(name)
{
}
ImageViewer::ImageViewer(const char* name, std::shared_ptr<TextureHolder> aTexture) :
	texture(aTexture->GetRawTexture()), EditorWindow(name)
{
}
...
void ImageViewer::RenderImGUi()
{
	...
	ImGuiTexInspect::InspectorFlags inspectorFlags = 0;
	if (flipX)  inspectorFlags |= ImGuiTexInspect::InspectorFlags_FlipX;
	if (flipY) inspectorFlags |= ImGuiTexInspect::InspectorFlags_FlipY;
	if (showWrap) inspectorFlags |= ImGuiTexInspect::InspectorFlags_ShowWrap;
	if (!forceNearestTexel) inspectorFlags |= ImGuiTexInspect::InspectorFlags_NoForceFilterNearest;

	auto v2 = ImVec2((float)texture->GetResolution().x, (float)texture->GetResolution().y);
	if (ImGuiTexInspect::BeginInspectorPanel("##ColorFilters", texture.get(), v2, inspectorFlags))
	{
