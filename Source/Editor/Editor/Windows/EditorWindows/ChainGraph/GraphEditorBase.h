...
#include <Tools/ImGui/MuninGraph/Graph/GlobalUID.h>
#include <Tools/ImGui/MuninGraph/Graph/GraphColor.h>
#include <Tools/ImGui/MuninGraph/Graph/GraphTypes.h>

namespace ImNodeEd = ax::NodeEditor;

template <typename GraphType, typename GraphNodeType, typename GraphEdgeType, typename GraphPinType,
          typename SchemaType>
...
template <typename GraphType, typename GraphNodeType, typename GraphEdgeType, typename GraphPinType,
          typename SchemaType>
std::shared_ptr<GraphNodeType> GraphEditorBase<GraphType, GraphNodeType, GraphEdgeType, GraphPinType, SchemaType>::
    CreateNode(const typename GraphType::SupportedNodeClass &aNodeClass)
{
...
template <typename GraphType, typename GraphNodeType, typename GraphEdgeType, typename GraphPinType,
          typename SchemaType>
void GraphEditorBase<GraphType, GraphNodeType, GraphEdgeType, GraphPinType, SchemaType>::RenderEdge(
    const GraphEdgeType &anEdge)
{
    const GraphColor typeColor = anEdge.Color.AsNormalized();
    ImNodeEd::Link(anEdge.EdgeId, anEdge.FromUID, anEdge.ToUID, {typeColor.R, typeColor.G, typeColor.B, typeColor.A},
                   anEdge.Thickness);
}
...
void GraphEditorBase<GraphType, GraphNodeType, GraphEdgeType, GraphPinType, SchemaType>::RenderBackgroundContextMenu()
{
    ...
    if (ImGui::BeginPopup("BackgroundContextMenu"))
    {
        ...
        myEditorState.Search.bgCtxtSearchFieldChanged =
            ImGui::InputText("##nodeSearch", &myEditorState.Search.bgCtxtSearchField);
                ...
                if (ImGui::TreeNodeEx(catName.c_str(), ImGuiTreeNodeFlags_SpanAvailWidth))
                {
                    for (const auto &item : catIt->second.Items)
                    {
                                              ...
                                              ImGuiTreeNodeFlags_Leaf | ImGuiTreeNodeFlags_SpanAvailWidth))
                        {
                            ...
                            {
                                const typename GraphType::SupportedNodeClass &type =
                                    GraphType::GetSupportedNodeTypes().find(item.Value)->second;
            ...
            if (myEditorState.Search.bgCtxtSearchFieldChanged)
            {
                ...
                for (const auto &[nodeName, nodeType] : supportedNodeTypes)
                {
                    myEditorState.Search.bgCtxtSearchFieldResults.push_back(
                        {nodeType.NodeTitle, nodeType.TypeName, 0, ""});

                    myEditorState.Search.bgCtxtSearchFieldResults.back().Rank =
                        LevenshteinDistance(myEditorState.Search.bgCtxtSearchFieldResults.back().Title,
                                            myEditorState.Search.bgCtxtSearchField);
                }

                std::sort(myEditorState.Search.bgCtxtSearchFieldResults.begin(),
                          myEditorState.Search.bgCtxtSearchFieldResults.end(),
...
template <typename GraphType, typename GraphNodeType, typename GraphEdgeType, typename GraphPinType,
          typename SchemaType>
void GraphEditorBase<GraphType, GraphNodeType, GraphEdgeType, GraphPinType, SchemaType>::Init(
    std::shared_ptr<GraphType> aGraph)
{
