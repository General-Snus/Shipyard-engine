...
void ScriptGraphEditor::RenderNode(const ScriptGraphNode* aNode)
{
	...
	if (aNode->IsExecNode() || !aNode->IsSimpleNode())
	{
		...
		{
			const ImTextureID funcTextureId = (void*)It->second->GetHandle(ViewType::SRV).first.ptr;
	...
	if (aNode->IsExecNode() || !aNode->IsSimpleNode())
	{
		...
		const ImTextureID nodeTextureId = (void*)myNodeHeaderTexture->GetHandle(ViewType::SRV).first.ptr;
	...
	if (isGetVarNode)
	{
		...
		const ImTextureID nodeTextureId = (void*)myGetterGradient->GetHandle(ViewType::SRV).first.ptr;
		nodeDrawList->AddImageRounded(nodeTextureId,
			nodeHeader.Min,
			nodeHeader.Max,
			ImVec2(0.1f,0.2f),ImVec2(0.8f,0.8f),
