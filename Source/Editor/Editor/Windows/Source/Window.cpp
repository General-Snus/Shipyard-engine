...
void Window::Messagebox(std::string title, std::string text, UINT messageBoxType)
{
	std::wstring wTitle = std::wstring(title.begin(), title.end());
	std::wstring wMessage = std::wstring(text.begin(), text.end());

	MessageBeep(MB_ICONERROR); // funny
	MessageBox(NULL, wMessage.c_str(), wTitle.c_str(), messageBoxType);
}
