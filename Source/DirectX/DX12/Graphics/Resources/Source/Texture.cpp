...
bool Texture::AllocateDepthTexture(const Vector2ui dimentions, const std::filesystem::path& name, FLOAT depth,
		UINT8 stencil, DXGI_FORMAT Format, D3D12_RESOURCE_FLAGS flags, D3D12_RESOURCE_STATES targetResourceState)
{
	OPTICK_EVENT();
	const auto width = dimentions.x;
	const auto height = dimentions.y;
	m_ClearColor.x = depth;
	m_ClearColor.y = stencil;
