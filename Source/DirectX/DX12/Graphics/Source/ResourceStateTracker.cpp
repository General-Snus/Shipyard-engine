...
void ResourceStateTracker::ResourceBarrier(const D3D12_RESOURCE_BARRIER& barrier)
{
	...
	if (lastKnownState.contains(transitionBarrier.pResource))
	{
		const auto& resourceState = lastKnownState.at(transitionBarrier.pResource);
	...
	lastKnownState[transitionBarrier.pResource].SetSubresourceState(transitionBarrier.Subresource, transitionBarrier.StateAfter);
}

void ResourceStateTracker::TransitionResource(ID3D12Resource* resource, D3D12_RESOURCE_STATES stateAfter,
											  unsigned subResource)
{
	OPTICK_EVENT("TransitionResource");
	if (resource)
	{
		ResourceBarrier(
...
{
	OPTICK_EVENT("FlushResourceBarriers");
...
{
	OPTICK_EVENT("CommitFinalResourceStates");
	assert(ms_IsLocked);

	for (const auto& resourceState : lastKnownState)
	{
...
{
	OPTICK_EVENT("ResoureTrackingLocked");
	ms_GlobalMutex.lock();
	ms_IsLocked = true;
}
...
{
	OPTICK_EVENT("ResoureTrackingUnlocked");
...
{
	OPTICK_EVENT("AddGlobalResourceState");
	if (resource != nullptr)
	{
...
{
	OPTICK_EVENT("SetSubresourceState");
